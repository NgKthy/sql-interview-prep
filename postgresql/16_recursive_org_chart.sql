-- ============================================================
-- 16_recursive_org_chart.sql  (PostgreSQL port)
-- Chủ đề: Recursive CTE — Org chart, Bill of Materials
-- Nguồn: LeetCode 1270 + PostgreSQL Recursive CTE
-- Khác biệt vs MySQL:
--   - PostgreSQL mặc định hỗ trợ RECURSIVE rất mạnh và chuẩn ANSI SQL
--   - Cú pháp ghép chuỗi dùng || hoặc CONCAT()
--   - Hỗ trợ kiểu dữ liệu TEXT cho path không giới hạn độ dài
-- ============================================================
...
-- Mở rộng BOM — tất cả component của Bike
WITH RECURSIVE explode AS (
    SELECT
        ...
        (parent_id || ' > ' || child_id)::TEXT AS path
    FROM bom
